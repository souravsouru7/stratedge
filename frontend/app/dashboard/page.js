...
function SecondaryPill({ label, value, color, delay=0 }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize:20, color, fontWeight:700, fontFamily:"'JetBrains Mono',monospace" }}>{value}</div>
    </div>
  );
}
...
function LoadingState() {
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"60vh", gap:14 }}>
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#0D9E6E" strokeWidth="2.5" style={{ animation:"spin 0.9s linear infinite" }}>
...
{/* Page Title */}
            ...
            <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap", animation:"fadeUp 0.5s ease 0.22s both" }}>
              <SecondaryPill label="WINNING TRADES" value={Math.round(stats.totalTrades*(stats.winRate/100))} color="#0D9E6E" delay={0.20}/>
              <SecondaryPill label="LOSING TRADES"  value={stats.totalTrades - Math.round(stats.totalTrades*(stats.winRate/100))} color="#D63B3B" delay={0.24}/>
              <SecondaryPill label="AVG WIN"        value={`$${(stats.totalProfit/Math.max(1,stats.totalTrades*(stats.winRate/100))).toFixed(0)}`} color="#0D9E6E" delay={0.28}/>
              <SecondaryPill label="AI INSIGHTS"    value={stats.totalTrades*3} color="#B8860B" delay={0.32}/>
            </div>
